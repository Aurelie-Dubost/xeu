    ...
    for leg in trade_structure:
        ...
        if strike is None and delta is not None:
            ...
            if not filtered_options.empty:
                ...
                print(f"No matching option found for type {option_type} with delta {delta}")
                continue
        else:
            filtered_options = option_chain[(option_chain['Type'] == option_type) & (option_chain['Strike'] == strike)]
            if not filtered_options.empty:
                premium = filtered_options['Premium'].iloc[0]
            else:
                print(f"No matching option found for type {option_type} and strike {strike}")
                continue
